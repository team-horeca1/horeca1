...
async function main() {
  ...
  const existingHeroSlide = await prisma.homepageHeroSlide.findFirst({ select: { id: true } });
  if (!existingHeroSlide) {
    await prisma.homepageHeroSlide.create({
      data: {
    ...
    console.log('  Homepage hero slide: seeded defaults');
  }
