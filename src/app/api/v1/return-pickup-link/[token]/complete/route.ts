// POST /api/v1/return-pickup-link/:token/complete — verify OTP → goods_received
...
async function postHandler(req: NextRequest) {
  try {
    ...
    const body = returnPickupLinkCompleteSchema.parse(await req.json());
    const data = await returnPickupLinkService.complete(token, body.otp);
