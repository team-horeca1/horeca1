...
export const GET = vendorOnly(async (req: NextRequest, ctx) => {
  try {
    ...
    const data = await warehouseService.listDispatches(voc.vendorId, voc.outletId);
...
export const POST = vendorOnly(async (req: NextRequest, ctx) => {
  try {
    ...
    const data = await warehouseService.createDispatch(voc.vendorId, voc.outletId, body);
