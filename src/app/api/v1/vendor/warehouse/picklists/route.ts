...
export const GET = vendorOnly(async (req: NextRequest, ctx) => {
  try {
    ...
    const data = await warehouseService.listPicklists(voc.vendorId, voc.outletId);
...
export const POST = vendorOnly(async (req: NextRequest, ctx) => {
  try {
    ...
    const data = await warehouseService.createPicklist(voc.vendorId, voc.outletId, body);
