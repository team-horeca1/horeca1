  ...
  return (
    <div className="space-y-5">
        ...
        <div className="divide-y divide-[#F5F5F5] border border-[#EEEEEE] rounded-[10px] overflow-hidden">
          {documents.map((doc) => (
            <div key={doc.id} className="flex items-center gap-3 px-4 py-3">
              ...
              {doc.status === 'verified' ? (
                <span className="flex items-center gap-1 text-[11px] font-bold text-success bg-success-light px-2 py-0.5 rounded-[5px] shrink-0"><CheckCircle2 size={11} /> Verified</span>
              ) : doc.status === 'rejected' ? (
              ...
              <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="text-[12px] font-bold text-primary hover:underline shrink-0">View</a>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <select value={docType} onChange={(e) => setDocType(e.target.value as VendorDocument['type'])} className="h-[44px] border border-[#EEEEEE] rounded-[10px] px-3 text-[14px] bg-white outline-none focus:border-primary/40">
        ...
        <button type="button" onClick={() => docFileRef.current?.click()} className="h-[44px] flex items-center gap-2 border border-dashed border-primary/40 rounded-[10px] px-4 text-[14px] text-left hover:bg-primary-light/40">
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={onUpload} disabled={uploadingDoc || !docFile} className="flex items-center gap-2 px-5 py-2.5 bg-primary text-white text-[13px] font-bold rounded-[10px] disabled:opacity-50">
