    ...
    try {
      ...
      await Promise.all([
        ...
        notifications.send({
          ...
          body: `A new order worth ₹${payload.totalAmount.toLocaleString('en-IN')} has been placed. Log into Horeca1 to confirm and fulfill it.`,
          referenceId: payload.orderId,
          referenceType: 'order',
        }),
        notifications.send({
  ...
  eventBus.on('VendorOnboarded', async (payload) => {
    try {
      ...
      await notifications.send({
        ...
        body: `Congratulations! "${payload.businessName}" has been verified and is now live on Horeca1.`,
