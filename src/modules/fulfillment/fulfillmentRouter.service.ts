...
export class FulfillmentRouterService {
  ...
  async resolveFulfillmentOutlet(input: FulfillmentRouteInput): Promise<string> {
    ...
    if (!vendor.multiWarehouseEnabled || outlets.length === 1) {
      await this.assertStockAtOutlet(primaryOutletId, input.items);
    ...
    const firstFail = primaryCheck.find((r) => !r.available);
    throw Errors.outOfStock(
      firstFail?.productName ?? 'Item',
      firstFail?.qtyAvailable ?? 0,
    );
  }
  ...
  private async assertStockAtOutlet(
    ...
    if (fail) throw Errors.outOfStock(fail.productName, fail.qtyAvailable);
  }
